...
abstract class ProfileDataSource {
  ...
  Future<UserModel> getprofilePhotoAndName({required String userId});
...
class ProfileDataSourceImpl implements ProfileDataSource {
  ...
  Future<void> blockUnblock({required String userId}) async {
    await api.put(EndPoints.blockunblockEndPoint, data: {"targetUserId": userId});
  }

  @override
  Future<void> changepassword(
      {required String oldPassword, required String newPassword}) async{
    await api.put(EndPoints.changepasswordEndPoint, data: {"oldPassword": oldPassword, "newPassword": newPassword});

  }

  @override
  Future<void> followUnfollowUser({required String userId}) async {
    await api.put(EndPoints.followunfollowEndPoint, data: {"targetUserId": userId});
  }

  @override
  Future<UserProfileModel> getUserProfile({required String userId}) async{
       print("ssssssssssssssssffdsfssss");
    print(userId);
    Response response = await api
        .get(EndPoints.getusersprofileEndPoint, queryParameters: {"userId": userId});
    UserProfileModel userProfileModel = UserProfileModel.fromResponse(response.data);
    return userProfileModel;
  }

  @override
  Future<UserModel> getprofilePhotoAndName({required String userId}) async {
    Response response = await api
        .get(EndPoints.getnameimageEndPoint, queryParameters: {"usersIds": userId});
    UserModel postModel = UserModel.fromResponse(response.data);
    return postModel;
  }
  ...
  Future<void> modifyprofile(
     ...
     List< String >?preferredTopics,
      ...
      String? city,
      String? about}) async {
    await api.patch(EndPoints.ModifyProfileEndPoint, data: {
      "files": userName,
      "userName": userName,
      "firstName": firstName,
      "lastName": lastName,
